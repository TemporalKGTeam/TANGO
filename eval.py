    ...
    with torch.no_grad():
        ...
        for (
            ...
            while rank_count < sub_tar[0].shape[0]:
                l, r = (
                    ...
                    if (rank_count + rank_group_num) <= sub_tar[0].shape[0]
                    else sub_tar[0].shape[0],
                )

                # push data onto gpu
                [sub_tar_, rel_tar_, obj_tar_, lab_tar_, indep_lab_] = push_data2(
                    sub_tar[0][l:r],
                    rel_tar[0][l:r],
                    obj_tar[0][l:r],
                    lab_tar[0][l:r, :],
                ...
                score = model.score_comp(
                    sub_tar_, rel_tar_, emb, model.odeblock.odefunc
                )
                ...
                ranks = ranks.float()
                results["count_raw"] = torch.numel(ranks) + results.get(
                    "count_raw", 0.0
                )
                results["mar_raw"] = torch.sum(ranks).item() + results.get(
                    "mar_raw", 0.0
                )
                results["mrr_raw"] = torch.sum(1.0 / ranks).item() + results.get(
                    "mrr_raw", 0.0
                )
                for k in range(10):
                    results["hits@{}_raw".format(k + 1)] = torch.numel(
                        ranks[ranks <= (k + 1)]
                    ) + results.get("hits@{}_raw".format(k + 1), 0.0)

                # time aware filtering
                target_score = score[b_range, obj_tar_]
                score = torch.where(
                    lab_tar_.byte(), -torch.ones_like(score) * 10000000, score
                )
                ...
                for k in range(10):
                    results["hits@{}".format(k + 1)] = torch.numel(
                        ranks[ranks <= (k + 1)]
                    ) + results.get("hits@{}".format(k + 1), 0.0)

                # time unaware filtering
                score = torch.where(
                    indep_lab_.byte(), -torch.ones_like(score) * 10000000, score
                )
                score[b_range, obj_tar_] = target_score
                ...
                ranks = ranks.float()
                results["count_ind"] = torch.numel(ranks) + results.get(
                    "count_ind", 0.0
                )
                results["mar_ind"] = torch.sum(ranks).item() + results.get(
                    "mar_ind", 0.0
                )
                results["mrr_ind"] = torch.sum(1.0 / ranks).item() + results.get(
                    "mrr_ind", 0.0
                )
                for k in range(10):
                    results["hits@{}_ind".format(k + 1)] = torch.numel(
                        ranks[ranks <= (k + 1)]
                    ) + results.get("hits@{}_ind".format(k + 1), 0.0)
        ...
        for k in range(10):
            results["hits@{}".format(k + 1)] = round(
                results["hits@{}".format(k + 1)] / results["count"], 5
            )
            results["hits@{}_raw".format(k + 1)] = round(
                results["hits@{}_raw".format(k + 1)] / results["count_raw"], 5
            )
            results["hits@{}_ind".format(k + 1)] = round(
                results["hits@{}_ind".format(k + 1)] / results["count_ind"], 5
            )
