...
_BASHFORTH_DIVISOR = [
    torch.tensor([b / divisor for b in bashforth], dtype=torch.float64)
    for bashforth, divisor in zip(_BASHFORTH_COEFFICIENTS, _DIVISOR)
]
_MOULTON_DIVISOR = [
    torch.tensor([m / divisor for m in moulton], dtype=torch.float64)
...
class AdamsBashforthMoulton(FixedGridODESolver):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert max_order <= _MAX_ORDER, "max_order must be at most {}".format(
            _MAX_ORDER
        )
        if max_order < _MIN_ORDER:
            warnings.warn(
                "max_order is below {}, so the solver reduces to `rk4`.".format(
                    _MIN_ORDER
                )
            )
    ...
    def _has_converged(self, y0, y1):
        ...
        error_ratio = _compute_error_ratio(
            torch.abs(y0 - y1), self.rtol, self.atol, y0, y1, _linf_norm
        )
    ...
    def _step_func(self, func, t, dt, y):
        ...
        if order < _MIN_ORDER - 1:
            ...
            dy = _dot_product(dt * bashforth_coeffs, self.prev_f).type_as(
                y
            ...
            if self.implicit:
                ...
                for _ in range(self.max_iters):
                    dy_old = dy
                    f = func(t + dt, y + dy)
                    dy = (dt * (moulton_coeffs[0]) * f).type_as(
                        y
                    ) + delta  # moulton is float64 so cast back
