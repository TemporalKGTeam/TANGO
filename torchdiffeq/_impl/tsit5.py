...
class Tsit5Solver(AdaptiveStepsizeODESolver):
    def __init__(
        ...
        **unused_kwargs,
        ...
        self.grid_points = tuple(
            _convert_to_tensor(point, dtype=torch.float64, device=y0[0].device)
    ...
    def before_integrate(self, t):
        if self.first_step is None:
            ...
            first_step = _convert_to_tensor(
                self.first_step, dtype=t.dtype, device=t.device
            )
        ...
        self.next_grid_index = min(
            bisect.bisect(self.grid_points, t[0]), len(self.grid_points) - 1
        )
    ...
    def _adaptive_tsit5_step(self, rk_state):
        ...
        on_grid = (
            ...
            and t0 < self.grid_points[self.next_grid_index] < t0 + dt
        )
        if on_grid:
