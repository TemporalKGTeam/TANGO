...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="mgcn",
        help="core layer function of the TANGO model",
    )
    parser.add_argument(
        "--score_func", type=str, default="tucker", help="score function"
    )
    parser.add_argument(
        "--core_layer", type=int, default=2, help="number of core function layers"
    )
    parser.add_argument(
        "--num_epoch", type=int, default=100, help="number of maximum epoch"
    )
    parser.add_argument(
        "--test_step",
        type=int,
        default=1,
        help="number of epochs after which we do evaluation",
    )
    parser.add_argument(
        "--input_step", type=int, default=4, help="number of input steps for ODEblock"
    )
    parser.add_argument(
        "--delta_step",
        type=int,
        default=0,
        help="number of steps between the last input snapshot and the prediction snapshot",
    )
    parser.add_argument(
        "--target_step", type=int, default=1, help="number of prediction snapshots"
    )
    parser.add_argument(
        "--initsize",
        type=int,
        default=200,
        help="size of initial representation dimension",
    )
    parser.add_argument(
        "--embsize", type=int, default=200, help="size of output embeddings"
    )
    parser.add_argument(
        "--hidsize",
        type=int,
        default=200,
        help="size of representation dimension in the core function",
    )
    parser.add_argument("--lr", type=float, default=0.001, help="learning rate")
    parser.add_argument("--solver", type=str, default="rk4", help="ODE solver")
    parser.add_argument(
        "--atol", type=float, default="1e-4", help="lower bound of the tolerance"
    )
    parser.add_argument(
        "--rtol", type=float, default="1e-3", help="higher bound of the tolerance"
    )
    parser.add_argument("--device", type=str, default="cuda:0", help="device name")
    parser.add_argument(
        "--dataset", type=str, default="ICEWS05-15", help="dataset name"
    )
    parser.add_argument(
        "--scale", type=float, default=0.1, help="scale the length of integration"
    )
    parser.add_argument("--dropout", type=float, default=0.3, help="dropout")
    parser.add_argument(
        "--bias",
        action="store_false",
        help="whether to use bias in relation specific transformation",
    )
    parser.add_argument(
        "--adjoint_flag", action="store_false", help="whether to use adjoint method"
    )
    parser.add_argument(
        "--opn",
        type=str,
        default="mult",
        help="composition operation to be used in MGCN",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="number of chebyshev nodes, without chebyshev approximation if cheby_grid=0",
    )
    ...
    parser.add_argument(
        "--jump", action="store_true", help="whether to use graph transition layer"
    )
    parser.add_argument(
        "--jump_init", type=float, default=0.01, help="weight of transition term"
    )
    parser.add_argument(
        "--activation", type=str, default="relu", help="activation function"
    )
    parser.add_argument(
        "--res", action="store_true", help="include residual MGCN layer"
    )
    parser.add_argument(
        "--rel_jump", action="store_true", help="include transition tensor"
    )
    parser.add_argument(
        "--induct_test", action="store_true", help="inductive link prediction"
    )
    parser.add_argument(
        "--test",
        action="store_true",
        help="store to start the test, otherwise start training",
    )
    ...
    if val_exist:
        if args.induct_test:
            (
                ...
                test_so2r,
                induct_tar,
            ) = setup_induct_test(args.dataset, logger, args.scale, args.input_step)
            ...
            trainl, testl, vall = (
                ...
                len(test_timestamps) - args.input_step,
                len(val_timestamps) - args.input_step,
            )
            adjlist = load_adjmtx(args.dataset)
            train_adjmtx, test_adjmtx, val_adjmtx = (
                adjlist[:trainl],
                adjlist[trainl + vall - args.input_step :],
                adjlist[trainl - args.input_step : trainl + vall],
            )
        ...
        (
            ...
            args.dataset, logger, args.initsize, args.scale, val_exist, args.input_step
        )
        trainl, testl = len(train_timestamps), len(test_timestamps) - args.input_step
        adjlist = load_adjmtx(args.dataset)
        train_adjmtx, test_adjmtx = (
            adjlist[:trainl],
            adjlist[trainl - args.input_step : trainl + testl],
        )

    if args.induct_test:
        test_dataset = TANGOtestDataset(
            args,
            test_triple,
            test_adj,
            test_adjmtx,
        ...
        test_loader = TANGOtestDataLoader(
            dataset=test_dataset, batch_size=1, shuffle=args.shuffle
        )
    else:
        train_dataset = TANGOtrainDataset(
            args,
            train_triple,
            train_adj,
            train_adjmtx,
        ...
        train_loader = TANGOtrainDataLoader(
            dataset=train_dataset, batch_size=1, shuffle=args.shuffle
        )
        ...
        if val_exist:
            ...
            val_loader = TANGOtestDataLoader(
    ...
    if args.induct_test == False and args.test == False:
        for epoch in range(args.num_epoch):
            ...
            if (epoch + 1) % args.test_step == 0:
                if val_exist:
                    split = "val"
                    results = predict(
                        val_loader, model, args, num_e, test_adjmtx, logger
                    )
                else:
                    split = "test"
                    results = predict(
                        test_loader, model, args, num_e, test_adjmtx, logger
                    )
                ...
                logger.info("===========RAW===========")
                logger.info(
                    "Epoch {}, HITS10 {}".format(epoch + 1, results["hits@10_raw"])
                )
                logger.info(
                    "Epoch {}, HITS3 {}".format(epoch + 1, results["hits@3_raw"])
                )
                logger.info(
                    "Epoch {}, HITS1 {}".format(epoch + 1, results["hits@1_raw"])
                )
                ...
                logger.info(
                    "Epoch {}, HITS10 {}".format(epoch + 1, results["hits@10_ind"])
                )
                logger.info(
                    "Epoch {}, HITS3 {}".format(epoch + 1, results["hits@3_ind"])
                )
                logger.info(
                    "Epoch {}, HITS1 {}".format(epoch + 1, results["hits@1_ind"])
                )
        ...
        if (
            args.induct_test
        ):  # inductive link prediction, run if you have a trained model
