...
class MGCNLayerWrapper(torch.nn.Module):
    def __init__(
        ...
        # define MGCN Layer
        self.conv1 = MGCNConvLayer(
            self.p.initsize, self.p.hidsize, act=self.act, params=self.p
        )
        self.conv2 = (
            MGCNConvLayer(self.p.hidsize, self.p.embsize, act=self.act, params=self.p)
    ...
    def set_jumpfunc(
        self, edge_id_jump, edge_w_jump, jumpfunc, jumpw=None, skip=False, rel_jump=None
    ):
    ...
    def forward(self, t, emb):
        ...
        jump_emb = emb.clone()
        if self.p.res:
            emb = emb + self.res * self.conv1(
                emb, self.edge_index, self.edge_type, self.num_e
            )
            ...
            emb = (
                (
                    emb
                    + self.res
                    * self.conv2(emb, self.edge_index, self.edge_type, self.num_e)
                )
                if self.p.core_layer == 2
                else emb
            )
        ...
        if self.p.jump:
            if not self.skip:
                if self.p.rel_jump:
                    ...
                    jump_res = self.jump(
                        jump_emb, self.edge_id_jump, dN=self.edge_w_jump
                    )
